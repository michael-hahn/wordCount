...
class DD_NonEx_NonIncr[T:ClassTag,K:ClassTag] {
  ...
  def test(inputRDD: RDD[T], testFunc: userTest_NonIncr[T], lm: LogManager, fh: FileHandler): Boolean = {
    testFunc.usrTest(inputRDD, lm, fh)
  }

  private def dd_helper(inputRDD: RDD[(T,K)],
                        numberOfPartitions: Int,
                        testFunc: userTest_NonIncr[T],
  ...
  def ddgen(inputRDD: RDD[(T,K)], testFunc: userTest_NonIncr[T], splitFunc: userSplit[T,K], lm: LogManager, fh: FileHandler): RDD[(T,K)] = {
